...
object Phase1ReadTypescript {
  def Pipeline(
    ...
    List(
      ...
      if (expandTypeMappings(libName)) new T.ExpandTypeMappings.After(LoopDetector.initial).visitTsParsedFile(scope.caching) else identity, // before ExtractInterfaces
      (
